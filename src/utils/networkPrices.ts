...
export class NetworkPrices {
  ...
  public getPrices = async function (
    ...
    if (token0EthPrice && !token1EthPrice)
      token1EthPrice = token0EthPrice
        .mul(priceUpdate.token1.price)
        .div(PRICE_MULTIPLIER);

    if (token1EthPrice && !token0EthPrice)
      token0EthPrice = token1EthPrice
        .mul(priceUpdate.token0.price)
        .div(PRICE_MULTIPLIER);

    if (!token0EthPrice && !token1EthPrice)
      throw new Error(
        `Couldn't fetch token prices ${priceUpdate.token1.address} ${priceUpdate.token0.address}`
      );
  ...
  public getTokenEthPrice = async function (
    ...
    if (this.cache[tokenAddress]?.timestamp > new Date().getTime() - 60000)
    ...
    try {
      ...
      throw new Error(
        `Couldn't fetch eth price of token: ${tokenAddress} ${e
          .toString()
          .substring(0, 400)} ...`
      );
    }
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (this.internalCache[url]?.timestamp + 60000 > currentTime) {
        ...
        await new Promise((r) =>
          setTimeout(
            () => r(undefined),
            currentTime < this.nextRequestAvalable
              ? this.nextRequestAvalable - currentTime
              : 0
          )
        );
        this.nextRequestAvalable =
          Math.max(this.nextRequestAvalable, currentTime) + 1000;
