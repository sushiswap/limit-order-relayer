...
export const getLimitOrderPairs = async (
  chainId: number
): Promise<IWatchPair[]> => {
  const tokens = (DEFAULT_TOKEN_LIST as ITokenList).tokens.filter(
    (token) => token.chainId === chainId
  );
  const tokensMainnet = (DEFAULT_TOKEN_LIST as ITokenList).tokens.filter(
  ...
  await Promise.all(
    limitOrderPairs.map(async ([token0Symbol, token1Symbol]) => {
      ...
      const token0mainnet = tokensMainnet.find(
        (token) => token.symbol === token0Symbol
      ) ?? { address: "" };
      const token1mainnet = tokensMainnet.find(
      ...
      if (token0 && token1) {
        ...
        throw new Error(
          `Error! Couldn't find token ${token0 ? token1Symbol : token0Symbol}`
        );
      }
    })
  );
  ...
  const poolBalances = await getPoolBalances(
    watchPairs,
    MyProvider.Instance.provider
  );

  const pairs = watchPairs.filter((pair, i) => {
    if (!poolBalances[i].token0.gt("0"))
      throw new Error(
        `Tokens aren't part of a pool: ${pair.token0.symbol} ${pair.token1.symbol}`
      );
    return true;
  });

  MyLogger.log(
    `Running on ${pairs.map(
      (wp) => ` ${wp.token0.symbol}-${wp.token1.symbol}`
    )}`
  );
