...
import { ROUND_UP_RECEIVER_ADDRESS } from "@sushiswap/limit-order-sdk";

export async function executeOrders(
  ordersData: ExecutableOrder[],
  gasPrice: BigNumber
): Promise<IExecutedOrder[]> {
  ...
  await Promise.all(
    ordersData.map(async (executableOrder) => {
      ...
      if (keepTokenIn) {
        const inDiff = executableOrder.inAmount
          .sub(executableOrder.minAmountIn)
          .div(10);
        amountExternal = executableOrder.minAmountIn.add(inDiff); // 10% of profit as slippage
      } else {
        amountExternal = executableOrder.outAmount.sub(
          executableOrder.outDiff.div(10)
        ); // 10% of profit as slippage
      }
      ...
      const fillOrder = new FillLimitOrder(
        ...
        ROUND_UP_RECEIVER_ADDRESS[ChainId.MATIC],
      ...
      const alreadyExecuted = ExecuteHelper.Instance.alreadyExecuted(
        executableOrder.limitOrderData.digest
      );
        ...
        if (executed) {
          ...
          MyLogger.log(
            `${transaction.hash}, gasPrice: ${
              parseFloat(gasPrice.div(1e8).toString()) / 10
            }, nonce: ${transaction.nonce}`
          );
        } else {
          ExecuteHelper.Instance.remove(executableOrder.limitOrderData.digest);
          MyLogger.log(
            `Gas estimation failed for: ${executableOrder.limitOrderData.digest}`
          );
        }
      } else {
...
export class ExecuteHelper {
  ...
  protected constructor() {
    const tokens = DEFAULT_TOKEN_LIST.tokens.filter(
      (token) => token.chainId === +process.env.CHAINID
    );

    this.profitTokens = getDesiredProfitToken(+process.env.CHAINID)
      .map(
        (tokenSymbol) =>
          tokens.find((token) => token.symbol === tokenSymbol).address
      )
      .reverse();

    if (this.profitTokens.length && this.profitTokens.indexOf(undefined) !== -1)
      MyLogger.log(
        `Error! Couldn't find profit token @ index ${this.profitTokens.indexOf(
          undefined
        )}`
      );
  }
  ...
  alreadyExecuted(digest: string) {
    this.executedOrders = this.executedOrders.filter(
      (o) => o.timestamp + this.timeBuffer > new Date().getTime()
    );

    const alreadyExecuted = this.executedOrders.some(
      (o) => o.digest === digest
    );
  ...
  keepTokenIn(tokenIn: string, tokenOut: string) {
    return (
      this.profitTokens.indexOf(tokenIn) > this.profitTokens.indexOf(tokenOut)
    );
  }
}
