...
export class Database {
  ...
  public async connectDB(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.database.on('error', () => {
        MyLogger.log('Error connecting to database')
        reject()
      })
    })
  }
  ...
  public async saveLimitOrder(limitOrder: ILimitOrder): Promise<ILimitOrder | void> {
    ...
    return model
      ...
      .then(() => {
        ...
        this.OrderCounterModel.updateOne({ timestamp: today }, { $inc: { counter: 1 } }, { upsert: true })
          .exec()
          .then()
      })
      .catch((err) => {
  ...
  public async getLimitOrders(tokenInPrice: BigNumber, pairAddress: string, tokenIn: string): Promise<ILimitOrder[]> {
    ...
    const limitOrders: ILimitOrder[] = (
      await this.LimitOrderModel.find({
        valid: true,
        pairAddress,
  ...
  public async invalidateLimitOrders(orders: ILimitOrder[]): Promise<{ n?: number }> {
    return this.LimitOrderModel.updateMany(
      { digest: { $in: orders.map((order) => order.digest) } },
      { valid: false }
    ).exec()
  }
